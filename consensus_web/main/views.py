...
from flask_login import login_required, session, current_user
...
def listar_itens_de_pauta_da_assembleia(num_assembleia):
    ...
    return nenhum_itempauta_ou_listar_com_op_voto\
         (u"Não há Itens de Pauta para a Assembléia selecionada", itens_pauta)
...
@main.route('/votar-item-pauta/', methods = ['POST'])
@login_required
def votar_itempauta():
...
def sugerir_itempauta():
    ...
    if form.validate_on_submit():
        ...
        if op.nome.__contains__("outras"):
            nova_op = incluir_opcao_voto(form, db)
            sugestao.op_voto = nova_op.num
        else:
            sugestao.op_voto = op.num
        anexos = incluir_anexos(session, db)
        sugestao.anexos = anexos
...
@permission_required(ConsensusTask.LISTAR_ITEM_PAUTA)
@main.route('/listar-sugestoes-sem-resposta/', methods = ['GET','POST'])
@login_required
def listar_sugestoes_sem_resposta():
    sugestoes = SugestaoItemPauta.query.filter(SugestaoItemPauta.status_aprovado == None).all()
    if not sugestoes:
        flash(u"Não há Sugestões de Item Pauta A RESPONDER")
        return redirect(url_for('main.index'))
    opcoes_voto = get_op_voto_por_sugestao(sugestoes)
    ids = [(it.num) for it in sugestoes]
...
def listar_sugestoes_reprovadas():
    sugestoes_reprovadas = SugestaoItemPauta.query.filter(SugestaoItemPauta.status_aprovado == 'N').all()
    return nenhum_itempauta_ou_listar_com_op_voto\
        (u"Não há Sugestões de Item Pauta REPROVADAS", sugestoes_reprovadas,
               "itempauta/listar_sugestoes_reprovadas.html")


@permission_required(ConsensusTask.RESPONDER_SUGESTAO_ITEM_PAUTA)
...
def reprovar_sugestao(num_itempauta):
    ...
    sugestao.status_aprovado = "N"
    sugestao.justif_reprovacao = justificativa
    db.session.add(sugestao)
    db.session.commit()
    flash(u"Sugestão de Item Pauta rejeitada com sucesso")
    return redirect(url_for('main.index'))
...
def desfazer_reprovacao(num_sugestao):
    ...
    sugestao_reprovada.status_aprovado = 'S'
...
def atribuir_sugestoes_a_assembleia():
    if request.method == 'GET':
        ...
        # alimenta o combo
        sugestoes_aprovadas = SugestaoItemPauta.query.filter(SugestaoItemPauta.status_aprovado == 'S').all()
        ...
        return render_template("itempauta/atribuir_a_assembleia.html",
                    sugestoes=sugestoes_aprovadas, assembleias_combo=assembleias, assembleias_heading=assembleias)
    else:
        ...
        for num_assembleia in assembleias_sugestoes.keys():
            for sug in assembleias_sugestoes[num_assembleia]:
                itempauta = ItemPauta(num_assembleia,sug)
